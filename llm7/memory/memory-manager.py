...
class ChunkingConfig:
    ...
    strategy: Literal["token", "recursive", "character", "code", "markdown", "semantic"] = "token"
    chunk_size: int = 500
...
class CodeBlockValidator(ChunkValidator):
    def validate(self, chunk: str) -> bool:
        """Validate that code blocks remain intact"""
...
class MemoryManager:
    ...
    def _initialize_chunking_strategy(self):
        ...
        if self.chunking_config.strategy == "token":
            ...
            self.text_splitter = TokenTextSplitter(
                encoding_name=self.chunking_config.model_name,
                **common_args
            )
            ...
            self.text_splitter = CharacterTextSplitter(
                separator=self.chunking_config.separators[0],
                **common_args
            )
            ...
            raise ValueError(f"Unsupported chunking strategy: {self.chunking_config.strategy}")

    def _validate_chunk(self, chunk: str) -> bool:
        """Run all configured validators on a chunk"""
        return all(validator(chunk) for validator in self.chunking_config.chunk_validators)
    ...
    def _chunk_text(self, text: str, content_type: str = "text") -> List[str]:
        ...
        if self.chunking_config.strategy == "semantic":
            ...
            while i < len(tokens):
                ...
                i += (self.chunking_config.chunk_size - self.chunking_config.chunk_overlap)
        else:
